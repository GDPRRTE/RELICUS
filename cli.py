...
def main():
    ...
    parser.add_argument(
        "-ch", "--chromium", action="store_true", help="Path to Chromium executable"
    )
    parser.add_argument(
        "-vpn", "--vpn", action="store_true", help="Path to VPN executable"
    )
    parser.add_argument(
        "--no-contact-scraper",
        action="store_true",
        help="Disable privacy contact scraper",
    )
    parser.add_argument(
    "--clean",
    action="store_true",
    help="Run crawl with fresh browser profile (no cache, cookies, storage)",
    )
    ...
    crawler = WebCrawler(
        ...
        logger=logger,
        enable_contact_scraper=not args.no_contact_scraper,
